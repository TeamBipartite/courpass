...
pipeline{
    ...
    stages{
        ...
        stage('Pytest tests'){
            steps {
                sh script: 'sed -i s/options.add_argument\\(\\"--headless=new\\"\\)// utils/calendar_scrape/calendar_scrape.py', label: 'workaround for headless Chrome not currently working'
